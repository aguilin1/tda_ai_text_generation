...
sentences = [human_1, human_2, machine_1, machine_2]
...
cls_embeddings = [query_vec_CLS_1, query_vec_CLS_2, query_vec_CLS_3, query_vec_CLS_4]
ind = 0
...
with torch.no_grad():
    for i in range(DATA_COUNT):
        ...
        query_vec = outputs.hidden_states[-1][0][0]
...
import pickle
with open("my_dataset_bert_cls_500.pickle", "wb") as pkl:
    pickle.dump(paragraph_emb, pkl)

print("Done with all ", DATA_COUNT, " paragraphs.", len(paragraph_emb), len(paragraph_emb[0]))
