...
# Pick either Vietoris-Rips or Alpha complex.
# We need to do this because SimplicialSets (ACSets backend) does not like us to keep injecting
# triangles with alternating orientations. (Makes Vietoris-Rips H2 computation expensive.).. Because we endup injecting
# too many triangles representing same void.
# In the end it does not matter whether we have 15770 voids (curtailed triangles) or 323613 voids (processing all alternating triangles)

# In case of Alpha it does make sense to process all triangles as we may get one triangle, followed by many (4-5 tetrahedrons) carrying 
# the same triangle. It is better to cancel out orientation.
# Julia RowEchelon has a bug where implementation does not use ceil or rounud so it gets stuck at Int(0.5) and then it does not know what to do
#       so code bails out with Load Error
# Using Linear Algebra Rank function to address this issue.
#
Vietoris_Rips = false
Alpha = true
...
# TODO: Tailor this 93 to number of inputs.
...
for (ind,v) in pairs(R_5_transpose)
  ...
  if i <= j
    ...
    if v < distance_threshold
      ...
      # put it in SimplexTree backing VRComplex/AlphaComplex
      insert!(B,[i,j], v)
    end
  end
end
...
############ G A T H E R #########################################
#
# Due to Balls intersecting at arbitrary times
# We can no longer assume linear in order construction as was the case 
# with Vietoris-Rips complex.
# For Alpha:
#           we may get (28, 30, 61), 
#           followed by (2, 28, 30, 31), (2, 28, 30, 32) (2, 28, 30, 61), (27, 28, 30, 61), (28, 30, 61, 62)
#           If we reject (2, 28, 30, 32), (2, 28, 30, 61) after seeing (2, 28, 30, 31) we may miss cancelling (28, 30, 61)
#           This did not happen in Vietoris-Rips as we got things in order, where first tetrahedrons showed up and then Tri.
#           For Alpha, Nerve Theorem guilds what can be added.
#
...
if Alpha == true
    dim_3_simplicial_complexes = va.Alpha.gathersimplices(B, 3)
end
...
# TODO: Not using removed_duplicates yet... we may have to consider this for BERT.
removed_duplicates = unique(dim_3_simplicial_complexes)
# ignore dense simplices.
process_this_simplex=false
simplex_key = Dict{Tuple{Int, Int, Int}, Int}()
...
for simplices in dim_3_simplicial_complexes
  ...
  if !haskey(simplex_key, (simplices[1], simplices[2], simplices[3]))
    ...
    #println("2 Saved Key Tri ", simplex_key[(simplices[1], simplices[2], simplices[3])])
  else
    global process_this_simplex=false
  end
  #println(simplices, "Processing - ? ", process_this_simplex, simplex_key[(simplices[1], simplices[2], simplices[3])])

  if process_this_simplex == false && Vietoris_Rips == true
      continue
  end
  # Only process out of order triangles for Alpha
  if process_this_simplex == false
    ...
    # In case of Alpha out of order scenarios
    if (size(simplices)[1] == 4)
        Tuple_1 = (simplices[1], simplices[2], simplices[3])
        Tuple_2 = (simplices[2], simplices[3], simplices[4])
	Tuple_3 = (simplices[1], simplices[3], simplices[4])
	Tuple_4 = (simplices[1], simplices[2], simplices[4])
	# If we detect already existing triangles, cancel them out...
	if haskey(simplex_key, Tuple_1)
            #println("Processing - ", Tuple_1)
	...
	if haskey(simplex_key, Tuple_2)
            #println("Processing - ", Tuple_2)
	...
	if haskey(simplex_key, Tuple_3)
            #println("Processing - ", Tuple_3)
	...
	if haskey(simplex_key, Tuple_4)
            #println("Processing - ", Tuple_4)
  ...
  if size(simplices)[1] == 5 || size(simplices)[1] == 4 
    ...
    if size(simplices)[1] == 4
	  ...
	  delete!(simplex_has_tri, [simplices[1], simplices[2], simplices[3]])
        ...
        if !haskey(simplex_has_tri, [simplices[1], simplices[2], simplices[4]])
          #println("124 - ", simplices[1], " - ", simplices[2], " - ", simplices[4]) 
          glue_triangle!(K5s, simplices[1], simplices[2], simplices[4], tri_orientation=true) #, tri_orientation=true)
	  ...
	  delete!(simplex_has_tri, [simplices[1], simplices[2], simplices[4]])
        ...
        if !haskey(simplex_has_tri, [simplices[1], simplices[3], simplices[4]])
          #println("134 - ", simplices[1], " - ", simplices[3], " - ", simplices[4]) 
          glue_triangle!(K5s, simplices[1], simplices[3], simplices[4], tri_orientation=true) #, tri_orientation=true)
          simplex_has_tri[[simplices[1], simplices[3], simplices[4]]] = 1
        else
          glue_triangle!(K5s, simplices[1], simplices[3], simplices[4], tri_orientation=false) #, tri_orientation=true)
	  delete!(simplex_has_tri, [simplices[1], simplices[3], simplices[4]])
        ...
        if !haskey(simplex_has_tri, [simplices[2], simplices[3], simplices[4]])
          #println("234 - ", simplices[2], " - ", simplices[3], " - ", simplices[4]) 
          glue_triangle!(K5s, simplices[2], simplices[3], simplices[4], tri_orientation=true) #, tri_orientation=true)
          simplex_has_tri[[simplices[2], simplices[3], simplices[4]]] = 1
        else
          glue_triangle!(K5s, simplices[2], simplices[3], simplices[4], tri_orientation=false) #, tri_orientation=true)
...
######   ∂3   ###################
K5s_Delta_3 = ∂(3, K5s)
#K5s_rref_Delta_3 = rref_with_pivots!(Matrix(ceil.(Int, K5s_Delta_3)))
#K5s_rref_Delta_3 = LinearAlgebra.rank(Matrix(ceil.(Int, K5s_Delta_3)))
K5s_rref_Delta_3 = LinearAlgebra.rank(Matrix(K5s_Delta_3))
println("Delta 3 Rank : ", K5s_rref_Delta_3)
K5s_rank_B_p_2 = K5s_rref_Delta_3 #size(K5s_rref_Delta_3[2])[1] # Number of Pivots

# convert to Z2
# My Z2 field.
#Z2 = ResidueRing(ZZ, 2)
#X = rand([0,1], size(K5s_Delta_3))
#println("K5s_Delta_3 : ", size(K5s_Delta_3), "X : ", size(X))
#A = matrix(Z2, X) 
#K5s_rank_B_p_2 = rank(A)
